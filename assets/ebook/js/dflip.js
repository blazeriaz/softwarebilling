...
/**
 * dFlip for FlipBook, TriFold
 */
var DFLIP = DFLIP || {};

var PRESENTATION = DFLIP;// old fallback
...
(function dFlip(DFLIP, $) {
	...
	DFLIP.PAGE_MODE = {SINGLE: 1, DOUBLE: 2};
	DFLIP.DIRECTION = {LTR: 1, RTL: 2};

	//defaults settings
	var defaults = DFLIP.defaults = {

		//sets if to use 3d or not - recommended is true(auto)
		webgl: true,

		/*dependency URLS*/
		//source link for PDF.JS file
		pdfjsSrc: ebook_path+"js/libs/pdf.min.js",
		//source link for PDF.WORKER.JS file
		pdfjsWorkerSrc: ebook_path+"js/libs/pdf.worker.min.js",

		//source link for THREE.JS file
		threejsSrc: ebook_path+"js/libs/three.min.js",
		//source link for MOCKUP.JS file
		mockupjsSrc: ebook_path+"js/libs/mockup.min.js",
		...
		//link to the images file that you want as background. supported files are jpgs,png. smaller files are preffered for performance
		backgroundImage: "blank",//"images/textures/el.jpg",
		//or any number like 5, 500. recommended: "auto"
		backgroundImageRepeat: "auto",

		//texture settings
		maxTextureSize: 1600,  //for pdf files only
		minTextureSize: 512, //for pdf files only
		...
		soundFile: ebook_path+"sound/turn2.mp3",
		soundEnable: true
	};
		...
		fixMouseEvent: function (event) {
			if (event) {
				...
				if (originalEvent.changedTouches && originalEvent.changedTouches.length > 0) {
					...
					event.movementX = touch.movementX;
					event.movementY = touch.movementY;
	...
	var SOURCE_TYPE = utils.SOURCE_TYPE,
		DISPLAY_TYPE = utils.DISPLAY_TYPE,
	...
	//resetTranslate = utils.resetTranslate,
	...
	var createUI = function (container, object) {
		...
		if (typeof options.source == 'string' && options.enableDownload == true) {
			...
			var download = ui.download = $('<a download target="_blank" class="' + downloadClass + '"><span>Download PDF File</span></a>');
		...
		var pageModeButton = ui.pageMode = $(html.div, {
			...
			html: "<span>Single Page Mode</span>"
		});
		start.html("<span>Goto First Page</span>");
		end.html("<span>Goto Last Page</span>");

		object.setPageMode(object.target.pageMode == DFLIP.PAGE_MODE.SINGLE)
		moreContainer.append(pageModeButton).append(start).append(end);
		...
		var outline = ui.outline = $(html.div, {
			...
			title:"Toggle Outline/Bookmark"
		});
		controls.append(sound).append(thumbnail).append(outline).append(altPrev).append(page).append(altNext).append(more).append(zoomIn).append(zoomOut).append(fullScreen);
	...
	var PreviewStage = void 0; //later updated via RegisterMockupObjects in case 3d is requested
	...
	function RegisterMockupObjects() {
		...
		PreviewStage = (function (_super) {
			...
			function PreviewStage(parameters) {
				...
				_this.dispose = function () {
					...
					_this.renderer.domElement.removeEventListener(mouseEvents.start, editor_mouseDown, false);
					_this.renderer.domElement.removeEventListener(mouseEvents.end, editor_mouseUp, false);
				...
				_this.renderer.domElement.addEventListener(mouseEvents.start, editor_mouseDown, false);
				_this.renderer.domElement.addEventListener(mouseEvents.end, editor_mouseUp, false);
		...
		var Book = (function (_super) {
			...
			function Book(parameters, stage) {
				...
				this.pageMode = isMobile ? DFLIP.PAGE_MODE.SINGLE : DFLIP.PAGE_MODE.DOUBLE;
	...
	var PreviewObject = (function (_super) {
		...
		PreviewObject.prototype = {
			...
			zoom: function (zoom) {
				...
				contentProvider.zoomScale = (zoom == true) ? 2 : 1;
				...
				if (this.stage !== void 0) {
					...
					//    transform: "scale(" + 1/*contentProvider.zoomScale*/ + ")"
					//});
					this.target.left = 0;
					this.target.top = 0;
				}
			...
			resize: function () {
				...
				var zoom = (contentProvider.zoomScale == 1 || contentProvider.zoomScale == void 0) ? 1
						: zoomViewport.height / parentHeight,//target.contentProvider.zoomScale,
					cameraZ, fov;

				contentProvider.checkViewportSize(zoom == 1 ? width * zoom : zoomViewport.width, zoom == 1
					? height * zoom : zoomViewport.height);
				...
				if (is3d) {
					...
					if (target !== void 0) {
						...
						target.wrapper.css({
							...
							height: height * zoom,
							marginTop: zoom == 1 ? (parentHeight - height) * zoom / 2 : 0
						});

						var stagePadding = target.stage.innerWidth() - target.stage.width();

						var shiftHeight = target.shiftHeight = height * (zoom - 1 ) / 2,
							shiftWidth = target.shiftWidth = zoom == 1 ? 1 :
															 (target.fullWidth * zoom - target.container.width() + stagePadding ) / 2;
			...
			setPageMode: function (isSingle) {
				if (isSingle == true) {
					...
					this.ui.pageMode.html("<span>Double Page Mode</span>");
					this.target.pageMode = DFLIP.PAGE_MODE.SINGLE;
				}
				else {
					this.ui.pageMode.removeClass(this.options.icons['doublepage']);
					this.ui.pageMode.html("<span>Single Page Mode</span>");
	...
	var ContentProvider = (function (_super) {
		...
		function ContentProvider(contentSource, callback, parameters) {
			...
			if (Array === _this.contentSource.constructor || Array.isArray(_this.contentSource) || _this.contentSource instanceof Array) { //case of images and html content
					...
					function () {
						...
						if (defaults.canvasToBlob == true) {
							canvas.toBlob(function (blob) {
								var src = PDFJS.createObjectURL(blob, "image/jpeg");
								log(performance.now() - start);
			...
			if (isThumb == true) {
				...
				if (bookPage !== void 0) {
					if (
						...
						bookPage.backImage(textureSrc, callbackFunction);
					}
					else {
						log(pageNumber + "rendered to front of " + bookPage.color);
						bookPage.frontImage(textureSrc, callbackFunction);
					}
				}
				else {
	...
	var PageCSS = (function () {
		...
		PageCSS.prototype = {
			...
			updatePoint: function (point) {
				...
				var radAngle = Math.atan2(point.y, (isRight ? point.rx : point.x));

				radAngle = Math.PI / 2 - limitAt(radAngle, 0, toRad(90));

				var correctionX = isRight ? point.x - pageWidth + (point.rx) / 2 : pageWidth - point.x / 2,
					refLength = Math.max(0, Math.sin(radAngle - Math.atan2(point.y / 2, correctionX)) * distOrigin(correctionX, point.y / 2)),

					foldLength = 0.5 * distOrigin(point.rx, point.y);

				var x = pageWidth - refLength * Math.sin(radAngle),
					y = refLength * Math.cos(radAngle),
					angle = isRight ? toDeg(radAngle) : 90 - toDeg(radAngle);
				...
				page.wrapper.css({
					transform: translateStr(isRight ? pageWidth - x : x, -y) + rotateStr(angle)
				});

				front.css({
					transform: rotateStr(-angle) + translateStr(isRight ? -pageWidth + x : -x, y)
				});
				...
				innerShadow.css({
					...
					backgroundImage: prefix.css + "linear-gradient( " + (isRight ? angle - 90
						: angle - 180) + "deg, rgba(0, 0, 0, 0.25) , rgb(0, 0, 0) " + foldLength * 0.7 + "px, rgb(255, 255, 255) " + foldLength + "px)"
				});
				...
				outerShadow.css({
					...
					backgroundImage: prefix.css + "linear-gradient( " + (isRight ? -angle - 90
						: -angle) + "deg, rgba(0, 0, 0,0) " + foldLength / 3 + "px, rgb(0, 0, 0) " + foldLength + "px)"
				});

				/*                $(".quick-hint").html(JSON.stringify({
				 x: (isRight ? pageWidth * 2 - point.x : point.x),
				 y: point.y,
				 angle: Math.round(angle),
				 dx: Math.round(x),
				 dy: Math.round(y),
				 fp: Math.round(foldLength),
			...
			updateAngle: function (angle, isRight) {
				...
				this.front.css({
					transform: "perspective(" + width + "px) "
			...
			tween: function (point) {
				...
				var init, first, mid,
					end = page.end = {x: isRight ? 0 : page.parent.fullWidth, y: 0};

				page.ease = page.isHard ? TWEEN.Easing.Quadratic.InOut : TWEEN.Easing.Linear.None;

				if (page.isHard == true) {
					init = page.init = {angle: isRight ? -0 : 0};
					end = page.end = {angle: isRight ? -180 : 180};
				} else {
					if (point == void 0) {
						init = page.init = {x: isRight ? page.parent.fullWidth : 0, y: 0};
						first = page.first = {
							x: (isRight ? 3 : 1 ) * page.parent.fullWidth / 4,
							y: page.parent.height / 5
						};
						mid = page.mid = {x: (isRight ? 1 : 3 ) * page.parent.fullWidth / 4, y: page.parent.height / 5};
					}
					else {
				...
				page.completeTween = page.completeTween || function (skipRefresh) {
					page.isFlipping = false;
				...
				if (page.isHard == true) {
					...
					page.currentTween = new TWEEN.Tween(init).delay(0)
						.to(first, page.parent.duration / 5)
						.onUpdate(function () {
							update(this);
						}).easing(TWEEN.Easing.Linear.None)
						.chain(
						new TWEEN.Tween(first)
							.to(mid, page.parent.duration / 3)
							.onUpdate(function () {
								update(this);
							}).easing(TWEEN.Easing.Linear.None)
							.chain(
							new TWEEN.Tween(mid)
								.to(end, page.parent.duration / 5)
								.onUpdate(function () {
									update(this);
								}).easing(TWEEN.Easing.Linear.None)
								.onComplete(page.completeTween)
	...
	var BookCSS = (function (_super) {
		...
		function pan(point, book) {
			var origin = book.startPoint;
			var scale = book.contentProvider.zoomScale;

			var left = (book.left + (point.raw.x - origin.raw.x)),
				top = (book.top + (point.raw.y - origin.raw.y));

			book.left = limitAt(left, -book.shiftWidth, book.shiftWidth);
			book.top = limitAt(top, -book.shiftHeight, book.shiftHeight);

			book.startPoint = point;

			//console.log(point, origin);
			book.stage.css({
				transform: "translate3d(" + book.left + "px," + book.top + "px,0)"
			});
		}
		...
		function BookCSS(parameters, container) {
			...
			_this.pageMode = isMobile ? DFLIP.PAGE_MODE.SINGLE : DFLIP.PAGE_MODE.DOUBLE;
			...
			var mouseMove = function (event) {
					...
					if ((event.touches !== void 0 || _this.isPanning == true) && _this.contentProvider.zoomScale !== 1)
						pan(point, _this);
					if (_this.skipDrag !== true) {
						...
						if (!hasFlipping(_this)) {
							if ((_this.dragPage !== void 0 ) || (distance < _this.foldSense)) {
				...
				onMouseWheel = function (event) {
					...
					if (delta) {
						if ((delta > 0 && zoom == false) || (delta < 0 && zoom == true)) {
							event.stopPropagation();
							event.preventDefault();
						}
						_this.previewObject.zoom(delta > 0);
			...
			if (containerDom) {
				...
				containerDom.addEventListener(mouseEvents.start, mouseDown, false);
				containerDom.addEventListener(mouseEvents.end, mouseUp, false);
			...
			this.dispose = function () {
				...
				containerDom.removeEventListener(mouseEvents.start, mouseDown, false);
				containerDom.removeEventListener(mouseEvents.end, mouseUp, false);
		...
		BookCSS.prototype.eventToPoint = function (event) {
			...
			return {
				isInside: left >= 0 && top >= 0 && left < fullWidth && top < height,
				x: left,
				y: top,
				fullWidth: fullWidth,
	...
	var TriFold = (function (_super) {
		...
		TriFold.prototype.init = function (webgl) {
			...
			if (webgl == true) {
				updateMockupJs(function () {
					...
					_this.stage.camera.position.set(0, 0, 400);
					_this.stage.orbitControl.maxDistance = 400;
		...
		TriFold.prototype.refresh = function () {
			...
			var _this = this;
			if (this._activePage > 6) this._activePage = 0;
			if (this._activePage < 0) this._activePage = 6;
			...
			switch (this._activePage) {
				...
				case 1:
					triFold.mid = void 0;
					triFold.end = {angle: 15, angle2: 178, rotation: 0, position: 2};
					break;
				case 2:
					triFold.mid = void 0;
					triFold.end = {angle: 15, angle2: 15, rotation: 0, position: 1};
					break;
				case 3:
					triFold.mid = {
						angle: isNext ? 15 : 90,
					...
					triFold.end = {angle: 15, angle2: 178, rotation: 0, position: 1};
					break;
				case 4:
					triFold.mid = {
						angle: 90,
					...
					break;
				case 5:
					triFold.mid = {angle: 2, angle2: 2, rotation: -Math.PI, position: triFold.width / 3};
...
var ThumbList = (function ThumbListClosure() {
	...
	ThumbList.prototype._renderChunk = function (node, from) {
		...
		if (!isEmpty && node.childNodes.length > (this.cachedItemsLen + 1)) {
			...
			/*if(node.childNodes.length- (delEnd - delStart) - this.cachedItemsLen > 4){
			 console.log("zombies");
			 }
			 console.log(lastIndex,from,start,end,delStart,delEnd ,node.childNodes.length- (delEnd - delStart) - this.cachedItemsLen);
			 }*/
			/*    node.appendChild(fragment);
			 for (countVar = from; countVar < finalItem; countVar++) {
			 if(this.addFn!==void 0){
			 this.addFn(countVar+1);
			 }*/
		}
		this.range.min = from;
		this.range.max = end;
