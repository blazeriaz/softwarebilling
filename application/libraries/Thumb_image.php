...
class Thumb_image
{
	...
	function __construct()
	{
		...
		$this->ci->load->config('thumb_image_config', TRUE);
	}
	...
	public function rimg($source, $params, $no_cache = FALSE)
	{
		...
		if (file_exists($this->ci->config->config['thumb_image_config']['base_path'] . '/' . $source))
		{
			$source = $this->ci->config->config['thumb_image_config']['base_path'] . '/' . $source;
		}
		else
		{
		...
		if (! is_file($source))
		{
			if(isset($params['no_image'])) 	$source = $params['no_image']."/0/no-image.png"; 
			else 	$source = "no-image.png"; 
			$source = trim($source); 
			if (file_exists($this->ci->config->config['thumb_image_config']['base_path'] . '/' . $source))
			{
				$source = $this->ci->config->config['thumb_image_config']['base_path'] . '/' . $source;
		...
		if(isset($params['dir'])) {
			 $this->create_directory($this->ci->config->config['thumb_image_config']['base_path'] . '/'.$params['dir']); 
			 $dir = '/'.$params['dir'].$dir;
		}
		// check if cache path exists
		if (! is_dir($this->ci->config->config['thumb_image_config']['base_path'] . $dir))
		{
			if (is_writable($this->ci->config->config['thumb_image_config']['base_path']))
			{
				$oldmask = umask(0);
				mkdir($this->ci->config->config['thumb_image_config']['base_path'] . $dir);
				umask($oldmask);
			}
			else
			{
				die('Unable to create cache dir in ' . $this->ci->config->config['thumb_image_config']['base_path']);
			}

		}

		// full path to final file
		$dst['file'] = $this->ci->config->config['thumb_image_config']['base_path'] . $dir . "/" . basename($source);
		...
		if (file_exists($dst['file']) AND $params['nocache'] !== TRUE)
		{
			...
			if ($params['return']) {
				return "{$this->ci->config->config['thumb_image_config']['base_url']}{$dir}/" . basename($dst['file']);
			} else {
				return "<img src=\"{$this->ci->config->config['thumb_image_config']['base_url']}{$dir}/" . basename($dst['file']) . "\" width=\"{$params['width']}\" height=\"{$params['height']}\" alt=\"{$params['alt']}\" {$extra_parameters}/>";
			}
		...
		if ($params['return'] === TRUE) {
			...
			return "<img src=\"{$this->ci->config->config['thumb_image_config']['base_url']}{$dir}/" . basename($dst['file']) . "\" width=\"{$dst['width']}\" height=\"{$dst['height']}\" alt=\"{$params['alt']}\" {$extra_parameters}/>";
		}
