...
abstract class AmChart
{
  ...
  // check if caps are set
  var cappedAxes = [];
  for(var i = 0; i < chart.valueAxes.length; i++) {
    var axis = chart.valueAxes[i];
    if (axis.capMinimum !== undefined || axis.capMaximum !== undefined)
      cappedAxes.push(axis);
  }
  
  // if caps are not set, were done here
  if (cappedAxes.length === 0)
    return;
  
  // add events to re-apply value axis range
  chart.addListener("zoomed", function(event) {
    for(var i = 0; i < cappedAxes.length; i++) {
      var axis = cappedAxes[i];
      if (axis.capMinimum !== undefined && (axis.minRR <= axis.capMinimum))
        axis.minimum = axis.capMinimum;
      else
        axis.minimum = undefined
      if (axis.capMaximum !== undefined && (axis.maxRR >= axis.capMaximum))
        axis.maximum = axis.capMaximum;
      else
