...
class Base_Model extends CI_Model
{
    ...
    ***/
    ...
    /*** Get the select List ***/
    function getSelectList($table, $where = array(), $default = array(), $fields = 'id,name',$order_by=null,$order_type=null,$keys = array())
    {
        ...
        foreach ($query->result_array() as $row) {
			if(!empty(keys)){
				$data[$row[$keys[0]]] = ucfirst($row[$keys[1]]);
			}else{
    ...
    /*** Get the array List ***/
    function getArrayList($table, $where = array(), $default = array(), $fields = 'id,name',$sort=NULL)
    {
        $data  = array();
        if(!is_array($fields)){
            $field = explode(',', $fields);
        ...
        $this->db->select($fields);
        $this->db->from($table);
        if(!is_array($fields)&&$fields=='id,name'){
            $this->db->order_by($field[1], 'asc');
        }
        if (!is_array($where)){
            if(!empty($where)){
    ...
    function getArrayList_type1($table, $where = array(), $default = array(), $first_field='id',$fields = 'name',$sort=NULL)
    {
        $data  = array();
        if(!is_array($fields)){
            $field = explode(',', $fields);
        }
        $this->db->select($first_field.','.$fields);
        $this->db->from($table);
        if(!is_array($fields)&&$fields=='id,name'){
            $this->db->order_by($field[1], 'asc');
        }
	...
	public function insert_or_update($data=null,$table_name=null,$condition=array()){
		if(!empty($condition)){
